...
func TestMergeTwoSortedLists(t *testing.T) {
	list1 := data_structure.NewListNodeFromArr([]int{1, 3, 5, 7})
	list2 := data_structure.NewListNodeFromArr([]int{2, 4, 6, 8})
	mergedList := mergeTwoSortedLists(list1, list2)
	ans := data_structure.NewListNodeFromArr([]int{1, 2, 3, 4, 5, 6, 7, 8})
